...
Phase 0 原始验收不要求真实的游戏知识检索或 LLM 应答；当前实现已经向前推进到内置 `sample-2048` 与自写 `sample-relay-station` GKP，可在 debug question route 中返回带来源的本地证据答案。
...
# adb forward、endpoint smoke、sample-2048 / sample-relay-station debug 问答和 latest-request 回读
...
- [ ] 执行上文「标准请求」curl，返回 `text` JSON；未知游戏会给出不确定性提示，内置样例可通过 `/debug/ask` 返回带来源的本地 evidence 答案。
...
- [ ] 执行 `./scripts/android_avd_smoke.sh` 后，两个内置样例 GKP debug 问答都返回 `pipeline_stage=evidence`、`llm_status=skipped`；latest-request 至少覆盖 `sample.relay.items`，输出日志中也能看到 `sample.2048.rules`。
...
- 原始 RetroArch AI Service 请求仍只提供截图/label/state，不包含玩家自然语言问题；真实问答入口暂时通过 `/debug/ask` 验证。
- 只有内置 `sample-2048` 与自写 `sample-relay-station` GKP 具备确定性本地答案；其他游戏仍会进入不确定性路径。
- LLM Adapter 已支持 BYOK DeepSeek/OpenAI-compatible，但 AnswerPolicy 要求有本地 evidence 才允许调用，默认不会无证据猜答案。
