...
默认监听地址：`127.0.0.1:8080`（loopback only）。
...
| `label` | string | 是 | 形如 `system__game` 的 ROM 标签，例如 `snes__super_mario_world`；用于将来路由到对应 GKP | ROM label `system__game`, used for GKP routing |
...
> Phase 0 仅实现 `text` 模式；其余模式将在后续阶段支持。RetroArch 端选择 `Subtitles` 时，URL 通常仍是 `?output=text`，以 `text` 字段返回即可。
